...
export default function ModalSeksjon({
  spørreundersøkelseId,
}: {
  spørreundersøkelseId: string;
}) {
  const searchParams = useSearchParams();
  const { data: spørreundersøkelseInfo } =
  ...
  const [skalÅpnePlanEtterQR, setSkalÅpnePlanEtterQR] = React.useState(
    loginModal === "true",
  );
  ...
  return (
    <>
      ...
      <PlanModal
        spørreundersøkelseId={spørreundersøkelseId}
        åpen={planModalÅpen}
        onClose={() => setPlanModalÅpen(false)}
      />
      <ModalVelger
        setPlanModalÅpen={setPlanModalÅpen}
        setQRmodalÅpen={setQRModalOgÅpnePlanOmNødvendig}
...
function ModalVelger({
  ...
  setPlanModalÅpen: (arg0: boolean) => void;
  setQRmodalÅpen: (arg0: boolean) => void;
  spørreundersøkelseInfo: SpørreundersøkelseInfoDto | undefined;
}) {
  ...
  if (spørreundersøkelseInfo?.type === "Evaluering") {
    ...
    return (
      <ActionMenu open={åpen} onOpenChange={setÅpen}>
        ...
        <ActionMenu.Content>
          <ActionMenu.Item onClick={() => setPlanModalÅpen(true)}>
            Samarbeidsplan
          </ActionMenu.Item>
          <ActionMenu.Item onClick={() => setQRmodalÅpen(true)}>
            QR-kode
          </ActionMenu.Item>
        </ActionMenu.Content>
      </ActionMenu>
