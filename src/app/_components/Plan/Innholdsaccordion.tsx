...
import { planStatusType, planTemaType, planUndertemaType } from "@/app/_types/Plantyper";


export default function InnholdsAccordion({
	tema
}: {
	tema: planTemaType;
}) {
	return (
		<Accordion className={planStyles.innholdAccordion} size="small" headingSize="xsmall">
...
function InnholdsStatusHeader({
	...
	return status ? (
		<span>
			{status[0]}{status.slice(1).toLocaleLowerCase()}
		</span>
	) : (
