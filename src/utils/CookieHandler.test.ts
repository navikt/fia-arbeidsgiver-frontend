...
describe("CookieHandler", () => {
  ...
  test("Sjekk at lagring av svar fungerer", () => {
    ...
    expect(CookieHandler.getSvarPåSpørsmål("spm1")).toStrictEqual(["svar1"]);
    expect(CookieHandler.getSvarPåSpørsmål("spm2")).toStrictEqual(["svar2"]);
    expect(CookieHandler.getSvarPåSpørsmål("spm3")).toStrictEqual(["svar3"]);

    CookieHandler.setSvarPåSpørsmålOgMarkerSomSisteSvarte("spm1", ["svar4"]);

    expect(CookieHandler.sisteSvarteSpørsmålId).toBe("spm1");

    expect(CookieHandler.getSvarPåSpørsmål("spm1")).toStrictEqual(["svar4"]);
  });
