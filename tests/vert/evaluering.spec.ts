...
const test = base.extend<object>({
  page: async ({ page }, use) => {
    ...
    await page.route(
      ...
      async (route) => {
        await route.fulfill({
          status: kontekstRoutes[0].variants[2].options.status,
          json: kontekstRoutes[0].variants[2].options.body,
        });
      },
    );
...
test.describe("Vert/evaluering", () => {
  test("rett innhold blir tegnet opp", async ({ page }) => {
    ...
    await page
      .getByLabel("Samarbeidsplan")
      .getByRole("heading", { name: "Partssamarbeid", exact: true })
      .click();
    await page
      .getByRole("heading", { name: "Utvikle partssamarbeidet" })
      .click();
    await page
      .getByRole("button", { name: "Utvikle partssamarbeidet 15." })
      .click();
    await page.getByText("Mål: Styrke og strukturere").click();
    await page
      .getByLabel("Samarbeidsplan")
      .getByRole("heading", { name: "Sykefraværsarbeid" })
      .click();
    ...
    await page
      .getByRole("button", { name: "Oppfølgingssamtaler 15. nov." })
      .click();
    await page
      .getByRole("button", { name: "Oppfølgingssamtaler 15. nov." })
      .click();
    ...
    await page
      .getByRole("heading", { name: "Oppfølging av arbeidsmiljø" })
      .click();
    await page
      .getByRole("button", { name: "Oppfølging av arbeidsmiljø" })
      .click();
